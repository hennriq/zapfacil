...
jest.mock('selenium-webdriver', () => ({
    ...
    By: {
        xpath: jest.fn((selector) => ({ selector })),
    },
    until: {
        elementLocated: jest.fn((locator) => ({ locator })),
    },
}));
jest.mock('selenium-webdriver/chrome', () => ({
    ...
    ServiceBuilder: jest.fn((driverPath) => ({ driverPath })),
}));
...
function resetSingleton() {
    ;
    ChromeDriverService_1.ChromeDriverService.instance = undefined;
}
describe('ChromeDriverService', () => {
    ...
    beforeEach(() => {
        ...
        resetSingleton();
        logger = createLogger();
        service = ChromeDriverService_1.ChromeDriverService.getInstance(logger);
    ...
    it('should keep a singleton instance', () => {
        const sameService = ChromeDriverService_1.ChromeDriverService.getInstance(createLogger());
        expect(sameService).toBe(service);
    });
    it('should initialize ChromeDriver with managed driver when available', async () => {
        await service.initialize();
        expect(fs_1.promises.mkdir).toHaveBeenCalledWith(expect.stringContaining('ZapFacil'), { recursive: true });
        expect(mockAddArguments).toHaveBeenCalledWith(expect.stringContaining('--user-data-dir='), '--profile-directory=ZapFacil', '--disable-extensions', '--disable-popup-blocking', '--no-first-run', '--no-default-browser-check');
    ...
    it('should reject initialization when managed driver is missing', async () => {
        ;
        fs_1.promises.access.mockRejectedValue(new Error('missing'));
    ...
    it('should reject browser actions before initialization', async () => {
        await expect(service.navigateTo('https://example.com')).rejects.toThrow('ChromeDriver not initialized');
...
//# sourceMappingURL=ChromeDriverService.test.js.map