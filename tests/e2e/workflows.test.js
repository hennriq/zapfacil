...
describe('E2E - Application Workflows', () => {
    let consoleSpy;
    beforeEach(() => {
        consoleSpy = (0, utils_1.spyOnConsole)();
    });
    afterEach(() => {
        consoleSpy.restore();
    });
    describe('Contact Import & Export Workflow', () => {
        it('should import contacts from CSV file', async () => {
            const mockContacts = (0, utils_1.createMockContacts)(3);
            // Simulate CSV import
        ...
        it('should export contacts to CSV file', async () => {
            const mockContacts = (0, utils_1.createMockContacts)(5);
        ...
        it('should handle import with validation warnings', async () => {
            const mockHandler = jest.fn().mockResolvedValue({
                canceled: false,
                contacts: (0, utils_1.createMockContacts)(2),
                validation: {
                    isValid: true,
                    errors: [],
    ...
    describe('Message Sending Workflow', () => {
        it('should send messages to multiple contacts', async () => {
            const contacts = (0, utils_1.createMockContacts)(5);
        ...
        it('should handle partial send failures gracefully', async () => {
            const contacts = (0, utils_1.createMockContacts)(4);
            const sendHandler = jest.fn().mockResolvedValue({
                results: [
        ...
        it('should retry on temporary failures', async () => {
            const contact = (0, utils_1.createMockContacts)(1)[0];
            let attemptCount = 0;
    ...
    describe('Authentication Workflow', () => {
        ...
        it('should handle authentication timeout', async () => {
            ...
            const initHandler = jest.fn(async () => {
                await new Promise((_, reject) => setTimeout(() => reject(new Error('QR code timeout')), timeoutMs));
            });
    ...
    describe('Logging & Monitoring Workflow', () => {
        it('should capture and display application logs', async () => {
            const logs = [
                (0, utils_1.createMockLogEntry)({ level: 'INFO', message: 'App started' }),
                (0, utils_1.createMockLogEntry)({ level: 'INFO', message: 'WhatsApp initialized' }),
                (0, utils_1.createMockLogEntry)({ level: 'SUCCESS', message: 'Contacts imported' }),
            ];
        ...
        it('should filter logs by level', async () => {
            const logs = [
                (0, utils_1.createMockLogEntry)({ level: 'INFO', message: 'Info message' }),
                (0, utils_1.createMockLogEntry)({ level: 'ERROR', message: 'Error message' }),
                (0, utils_1.createMockLogEntry)({ level: 'WARN', message: 'Warning message' }),
            ];
        ...
        it('should maintain log history limit', () => {
            const maxLogs = 100;
            const logs = Array.from({ length: 150 }, (_, i) => (0, utils_1.createMockLogEntry)({ message: `Log ${i}` }));
    ...
    describe('Error Recovery Workflow', () => {
        ...
        it('should log errors and continue operation', async () => {
            ...
            try {
                ...
                mockLogger.error(`Caught error: ${error.message}`);
